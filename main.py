...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Upsampling & Skip layer
    up1 = tf.layers.conv2d_transpose(
        layer7_1X1, num_classes, 4, strides=(2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    skip1 = tf.add(up1, layer4_1X1)

    up2 = tf.layers.conv2d_transpose(
        skip1, num_classes, 4, strides=(2, 2), padding='same', 
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    skip2 = tf.add(up2, layer3_1X1)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss += 0.01 * sum(reg_losses)
